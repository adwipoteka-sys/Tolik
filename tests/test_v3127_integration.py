...
def test_v3127_autonomous_run_executes_ranked_growth_paths_and_validates_follow_on_skills(tmp_path, capsys):
    ...
    assert "growth planner selected path: grounded_navigation -> spatial_route_composition -> route_mission_briefing" in captured
    assert "scheduled follow-on growth goal" in captured
    ...
    assert "completed growth path: grounded_navigation -> navigation_route_explanation" in captured

    portfolio_dir = runtime_dir / "ledger" / "capability_portfolio"
    states = {
        json.loads(path.read_text(encoding="utf-8"))["capability"]: json.loads(path.read_text(encoding="utf-8"))
        for path in portfolio_dir.glob("*.json")
    }

    assert states["grounded_navigation"]["maturity_stage"] == "transfer_validated"
    assert states["spatial_route_composition"]["maturity_stage"] == "transfer_validated"
    assert states["route_mission_briefing"]["maturity_stage"] == "transfer_validated"
