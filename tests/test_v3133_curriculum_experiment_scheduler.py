...
def test_scheduler_materializes_and_executes_response_risk_model_upgrade(tmp_path) -> None:
    ...
    assert [step.name for step in plan.steps[:3]] == [
        "stage_model_candidate",
        "train_model_candidate",
        "run_model_regression_gate",
    ]
    ...
    scheduler.record_outcome(selected.proposal_id, success=True, current_cycle=1, note="automl_finalized")
    assert planning.response_planning_policy == "adaptive_risk_model"
